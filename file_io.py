...
def generate_model_input_iad(placeholders, data_source, sess, c3d_map_depth):
	'''
	...
	data_ratio = float(np_values["length"])/time_frames
	buffer_len = (time_frames-np_values["length"]) * num_features

	img_data = np.pad(np_values["img"], 
									((0,0), (0,buffer_len)), 
									'constant', 
									constant_values=(0,0))
	img_data = img_data.reshape((num_features, time_frames, 1))
	img_data = np.expand_dims(img_data, axis=0)


	label = np_values["verb"][0]
	onehot = np.zeros((label.size, VERB_CLASSES))
	onehot[np.arange(label.size), label] = 1

	#place in dict

	placeholder_values = { placeholders["itr_in"]: img_data,
						   placeholders["system_out"]: onehot}
...
def parse_sequence_example(filename_queue):
	...
	sequence_data = {
		"img_raw": tf.decode_raw(sequence_parsed["img_raw"], tf.float32)# float values for IAD
	}
